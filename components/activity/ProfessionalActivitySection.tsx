...
import { getProfessionalDashboardData } from '@/lib/activity/actions';
import type { ProfessionalDashboardData } from '@/lib/activity/types';
...
export function ProfessionalActivitySection() {
  ...
  useEffect(() => {
    startTransition(async () => {
      const result = await getProfessionalDashboardData();
  ...
  if (isPending || !data) {
    return <ProfessionalActivitySkeleton />;
  }

  const { stats, recentActivity, recentCollections } = data;
...
function ProfessionalActivitySkeleton() {
  return (
