...
- Cloud storage (Cloudinary, S3) - _Reason: adds complexity, migrate when needed_
- Video file upload - _Reason: use external hosting (YouTube/Vimeo)_
- AI-powered image tagging - _Reason: not essential for MVP_
- Face detection/auto-cropping - _Reason: manual cropping sufficient_
- Watermarking - _Reason: not required by stakeholders_
- Chunked uploads - _Reason: 5MB limit makes this unnecessary_
...
- **Browser Drag-Drop API**: Varies across browsers, need fallback
  - _Mitigation_: Include traditional file input as fallback
- **Large File Uploads**: May timeout on slow connections
  - _Mitigation_: 5MB limit, show clear progress
- **Image Processing Time**: Sharp can be slow for large images
  - _Mitigation_: Show processing state after upload completes
- **Concurrent Uploads**: Multiple files at once
  - _Mitigation_: Queue uploads, process one at a time
